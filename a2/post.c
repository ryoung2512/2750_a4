...
#include <stdio.h>
struct PostEntry { 
    struct userPost * ( *PostEntryformatEntrycccc ) ( char * sender , char * stream , char * date , char * text );
    void ( *PostEntrygetTimeDatec ) ( char * currentTime );
    void ( *PostEntrysubmitPost ) ( struct userPost * p );
    void ( *PostEntryfreePost ) ( struct userPost * p );
};
...
void constructorPostEntry ( struct PostEntry *PostEntryvar ) { 
    ...
    PostEntryvar->PostEntryfreePost = PostEntryfreePost;
} 
int main ( int argc , char * argv[]) { 
    ...
    myPostEntry.PostEntrygetTimeDatec ( currentTime );
    post = myPostEntry.PostEntryformatEntrycccc ( sender , stream , currentTime , text );
    myPostEntry.PostEntrysubmitPost ( post );
    myPostEntry.PostEntryfreePost ( post );
    printf ( "</body>\n" );
    printf ( "</html>\n" );
    return 0;
}
